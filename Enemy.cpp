...
void Enemy::capNhat(float deltaTime) {
    ...
    // Không di xuyên qua bomb chua n?
    if (!chamTuong) {
    ...
    // N?u bomb dang n? ? ki?m tra vùng n?
    for (size_t i = 0; i < QuanLyBomb.size(); ++i) {
        const Bomb &b = QuanLyBomb[i];
        if (!b.dangNo) continue;

        if (c1 >= b.c3 + 64 * b.phamVi) continue;
        if (c3 <= b.c1 - 64 * b.phamVi) continue;
        if (c2 >= b.c4 + 64 * b.phamVi) continue;
        if (c4 <= b.c2 - 64 * b.phamVi) continue;
